...
const ANALYSIS_PROMPTS = {
  ...
  // 종합 투자심사보고서 생성
  FULL_REPORT: `당신은 벤처캐피탈의 시니어 투자심사역입니다. 
아래 제공된 기업 자료를 분석하여 상세한 투자심사보고서를 작성해주세요.
...
JSON이 아닌 마크다운 형식으로 작성해주세요.`,

  // 비실명 티저 생성
  TEASER: `당신은 벤처캐피탈의 딜소싱 담당자입니다.
아래 기업 분석 자료를 바탕으로 투자자들의 관심을 끌 수 있는 비실명 티저를 작성해주세요.
...
마크다운 형식으로 작성해주세요.`,
  ...
  // 빠른 요약 (간단 분석용)
  QUICK_SUMMARY: `아래 문서를 분석하여 다음 항목을 JSON 형식으로 추출해주세요:
...
## 추출 항목
{
  ...
  "strengths": ["강점 1", "강점 2"],
  "risks": ["리스크 1", "리스크 2"],
  "investment_stage": "투자 단계 (Seed/Series A/B/C)",
